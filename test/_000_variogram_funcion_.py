...
class Variogram:
    ...
    def __init__(self):
        ...
        self.bounds = {'nugget sill': [0., 100.],
                       'spherical vg_range': [0., 100000.],
                       'spherical sill': [0., 1000.],
                       'exponential vg_range': [0., 100000.],
                       'exponential sill': [0., 1000.],
                       'mat v': [0.5, 50.],
                       'mat vg_range': [0., 100000.],
                       'mat sill': [0., 20.],
                       'gauss vg_range': [0., 100000.],
                       'gauss sill': [0., 1000.]}
    ...
    def calc_expervar(self):
        ...
        for idx_classes in range(classes_bounds.shape[0]):
            ...
            for idx_pair in range(idx1.shape[0]):
                ...
                # Calculate the (z(s)-z(s+h))**2 values and the number of
                # values
...
# if idx_classes%10==0: print 'class',idx_classes,'finished'
    ...
    def setTheovar(self, theovartype):
        ...
        if 'composed' not in self.theovartype:
            ...
            if 'matern' not in self.theovartype:
                if 'mat vg_range' in self.parameters.keys():
                    self.parameters.pop('mat vg_range')
                    self.parameters.pop('mat sill')
                    self.parameters.pop('mat v')
            if 'gauss' not in self.theovartype:
                if 'gauss vg_range' in self.parameters.keys():
                    self.parameters.pop('gauss vg_range')
                    self.parameters.pop('gauss sill')
            if 'spherical' not in self.theovartype:
                if 'spherical vg_range' in self.parameters.keys():
                    self.parameters.pop('spherical vg_range')
                    self.parameters.pop('spherical sill')
            if 'exponential' not in self.theovartype:
                if 'exponential vg_range' in self.parameters.keys():
                    self.parameters.pop('exponential vg_range')
    ...
    def calc_theovar(self, xvalues):
        ...
        - spherical --> sill spherical, vg_range spherical
        - exponential --> sill exponential, vg_range exponential
        - matern --> v mat, sill mat, vg_range mat
        - gauss --> sill gauss, vg_range gauss
        """
        ...
        # variogram composed of certain variograms
            ...
            if 'gauss' in self.theovartype:
                self.theovar += calc_gaussvar(self.parameters['gauss vg_range'],
                                              self.parameters['gauss sill'],
                                              xvalues)
    ...
    def fit_theovar(self):
        ...
        while ((vario_ok == False) & (n_iter < max_iter)):
            ...
            # alphabetical order !
            if 'exponential' in self.theovartype:
                opt_params['exponential vg_range'] = params[0]
                opt_params['exponential sill'] = params[1]
                param_index += 2
            if 'gauss' in self.theovartype:
                opt_params['gauss vg_range'] = params[param_index]
                opt_params['gauss sill'] = params[param_index + 1]
                param_index += 2
            if 'matern' in self.theovartype:
                opt_params['mat vg_range'] = params[param_index]
            ...
            if 'nugget' in self.theovartype:
                ...
                param_index += 1
            if 'spherical' in self.theovartype:
                opt_params['spherical vg_range'] = params[param_index]
            ...
            # if sill is 0 vg_range is also zero and the other way round
            for ikey, key in enumerate(opt_params.keys()):
                if 'sill' in key:
                    model = key.split()[0]
                    if opt_params[key] == 0.:
                        opt_params[model + ' vg_range'] = 0.
                if 'vg_range' in key:
                    model = key.split()[0]
                    if opt_params[key] == 0.:
        ...
        for ikey, key in enumerate(self.parameters.keys()):
            if 'sill' in key:
                ...
                if self.parameters[key] == 0.:
                    if model != 'nugget':
                        self.parameters[model + ' vg_range'] = 0.
                        if model == 'mat':
    ...
    def get_params_name(self):
        ...
        for imodel, model in enumerate(vario_models):
            ...
            # Exponential model
            if 'Exp' in model:
                vg_range = np.float(model.split('Exp')[1].split('(')[1].split(
                    ')')[0])
                Sill = np.float(model.split('Exp')[0])
                self.parameters['exponential vg_range'] = vg_range
                self.parameters['exponential sill'] = Sill
            # Gaussian model
            if 'Gau' in model:
                vg_range = np.float(model.split('Gau')[1].split('(')[1].split(
                    ')')[0])
                Sill = np.float(model.split('Gau')[0])
                self.parameters['gauss vg_range'] = vg_range
                self.parameters['gauss sill'] = Sill
            # Sperical model
            if 'Sph' in model:
                vg_range = np.float(model.split('Sph')[1].split('(')[1].split(
                    ')')[0])
                Sill = np.float(model.split('Sph')[0])
                self.parameters['spherical vg_range'] = vg_range
                self.parameters['spherical sill'] = Sill
            # Matern model
            if 'Mat' in model:
                vg_range = np.float(model.split('Mat')[1].split('(')[1].split(
                    ')')[0])
                Sill = np.float(model.split('Mat')[0])
                V = np.float(model.split('Mat')[1].split('^')[1])
                self.parameters['mat vg_range'] = vg_range
...
def opt_theovar(params, distances, expervariogram, sillbound, nb_dec,
    ...
    if 'exponential' in theovartype:
        ...
        sumsill += params[1]
        if params[param_index] > rangebound:
            obj = 1000000.
        param_index += 2
...
def calc_sphericalvar(vg_range, sill, distances):
    """
    ...
    # 1. For distances smaller than or equal the vg_range
    smlt_range = (distances <= vg_range)
    if vg_range > 1.e-100:
        dist1 = distances[smlt_range]
        theovar[smlt_range] = sill * \
            (1.5 * (dist1 / vg_range) - 0.5 * (dist1**3. / vg_range**3.))
    else:
        theovar[smlt_range] = 0.

    # 2. For distances greater than the vg_range
    gtt_range = (distances > vg_range)
    theovar[gtt_range] = sill
    return theovar


def calc_expvar(vg_range, sill, distances):
    """ Calculates the exponential variogram.
    """
    if vg_range != 0.:
        theovar = sill * (1. - np.exp(-distances / vg_range))
    else:
        theovar = np.zeros(distances.shape[0])
    return theovar


def calc_matvar(v, vg_range, sill, distances):
    """
    ...
    if v > 100:
        theovar = calc_gaussvar(vg_range, sill, distances)
    elif v == 0.5:
        theovar = calc_expvar(vg_range, sill, distances)
    else:
        bessel = sps.kv     # modified bessel function of second kind of order v
        gamma = sps.gamma  # Gamma function

        factor1 = 1. / ((2.**(v - 1.)) * gamma(v))
        factor2 = (distances / vg_range)**v
        factor3 = bessel(v, distances / vg_range)
        theovar = sill * (1. - (factor1 * factor2 * factor3))

        # set nan-values at distances=0 to sill
        # theovar[distances>vg_range]=sill
        theovar[distances < 0.000001] = 0.
    return theovar


def calc_gaussvar(vg_range, sill, distances):
    """ Calculates the gaussian variogram.
    """
    if vg_range != 0.:
        theovar = sill * (1. - np.exp(-(distances**2.) / (vg_range**2)))
    else:
        theovar = np.zeros(distances.shape[0])
    return theovar
