...
for temp_agg in resample_frequencies:
    ...
    # DWD VG
    dwd_in_vgs_df = pd.read_csv(path_to_dwd_vgs,
                                index_col=0,
                                sep=';',
                                parse_dates=True,
                                infer_datetime_format=True,
    ...
    for idx_lst_comb in range(len(shuffled_dwd_stns_10stn)):
        ...
        for stn_dwd_id in stn_comb:
            ...
            for event_date in dwd_in_extremes_df.index.intersection(
                    dwd_in_vgs_df.index):
...
#                 vgs_model_dwd = fit_vg_list[0]
                vgs_model_dwd = dwd_in_vgs_df.loc[event_date, 1]
                if ('Nug' in vgs_model_dwd or len(vgs_model_dwd) == 0) and (
                    ...
                    try:
                        for i in range(1, 3):
                            vgs_model_dwd = dwd_in_vgs_df.loc[event_date, i]
...
#                 coords = np.array([(x, y) for x, y in zip(dwd_xcoords,
#                                       dwd_ycoords)])
...
#
#                 print(V)
#                 vg_sill = V.parameters[1]
#                 vg_range = V.parameters[0]
#                 V.plot()
#                 vgs_model_dwd = str(vg_sill) + ' ' + 'Exp(%s)' % vg_range
        ...
        df_interpolated_dwd_netatmos_comb.to_csv(out_plots_path / (
            'interpolated_rainfall_dwd_%s_data_%s_using_dwd_netamo_grp_%d_.csv'
            % (temp_agg, title_, idx_lst_comb)),
            sep=';', float_format='%0.2f')

        df_interpolated_dwd_only.to_csv(out_plots_path / (
            'interpolated_rainfall_dwd_%s_data_%s_using_dwd_only_grp_%d_.csv'
            % (temp_agg, title_, idx_lst_comb)),
            sep=';', float_format='%0.2f')

        df_interpolated_netatmo_only.to_csv(out_plots_path / (
            'interpolated_rainfall_dwd_%s_data_%s_using_netamo_only_grp_%d_.csv'
            % (temp_agg, title_, idx_lst_comb)),
            sep=';', float_format='%0.2f')
