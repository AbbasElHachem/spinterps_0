...
use_netatmo_zeros_on_event = True  # use 0 ppt information from Netatmos
...
if use_netatmo_zeros_on_event:
    path_to_netatmo_gd_stns_0_ppt = (
        main_dir / r'plots_NetAtmo_ppt_DWD_ppt_correlation_' /
        (r'keep_stns_all_neighbor_50_per_60min_s0_%s_.csv'
         % _acc_))
...
# Netatmo first filter
if use_netatmo_zeros_on_event:
    df_zeros_stns = pd.read_csv(path_to_netatmo_gd_stns_0_ppt,
                                index_col=0,
                                sep=';',
                                encoding='utf-8')
...
#                        (r'vg_strs_dwd_daily_ppt_.csv'))
    ...
    # netatmo with 0s as usefull information
    if use_netatmo_zeros_on_event:
        print('\n**using Netatmo zero info stns**')
        zeros_netatmo_stns = df_zeros_stns.loc[:, 'Stations'].values.ravel()
        zeros_gd_stns = netatmo_in_vals_df.columns.intersection(
            zeros_netatmo_stns)
        netatmo_in_vals_df_zeros = netatmo_in_vals_df.loc[:, zeros_gd_stns]
        netatmo_in_ppt_vals_df_zeros = netatmo_in_ppt_vals_df.loc[:, zeros_gd_stns]
...
#                   pprint.pformat(stn_comb))
            ...
            for stn_nbr, stn_dwd_id in enumerate(stn_comb):
                ...
                x_dwd_all_ngbrs = dwd_in_coords_df.loc[
                    ppt_dwd_vals_nona.index, 'X'].values
                y_dwd_all_ngbrs = dwd_in_coords_df.loc[
                ...
                netatmo_df_zeros = netatmo_in_ppt_vals_df_zeros.loc[
                    event_date,
                    sorted_distances_ppt_dwd.index].dropna(
                    how='all')

                # get vg model for this day
                vgs_model_dwd_ppt = df_vgs_extremes.loc[event_date, 'vg_model']
                ...
                # TODO: check use only if dwd VG Q
                    ...
                    if netatmo_df.size > 0:
                        ...
                        # add netatmo stations with 0 values
                        if netatmo_df_zeros.size > 0:
                            netatmo_ppt_zeros = []
                            x_netatmo_ppt_zeros = []
                            y_netatmo_ppt_zeros = []

                            for iz, (xzero, yzero) in enumerate(
                                zip(netatmo_in_coords_df.loc[
                                    netatmo_df_zeros.index, 'X'].values,
                                    netatmo_in_coords_df.loc[
                                    netatmo_df_zeros.index, 'Y'].values)):
                                ...
                                if (xzero not in
                                    ...
                                    ppt_zeros = netatmo_df_zeros.iloc[iz]
                                    if ppt_zeros == 0:
                                        netatmo_ppt_zeros.append(ppt_zeros)
                                        x_netatmo_ppt_zeros.append(xzero)
                                        y_netatmo_ppt_zeros.append(yzero)

                                        print('+-addiding zeros-+:  ',
                                              len(netatmo_ppt_zeros))

                        else:
                            netatmo_ppt_zeros = []
                            x_netatmo_ppt_zeros = np.array([])
                            y_netatmo_ppt_zeros = np.array([])
                        netatmo_ppt_zeros = np.array(netatmo_ppt_zeros)
                        x_netatmo_ppt_zeros = np.array(x_netatmo_ppt_zeros)
                        y_netatmo_ppt_zeros = np.array(y_netatmo_ppt_zeros)
                        #======================================================
