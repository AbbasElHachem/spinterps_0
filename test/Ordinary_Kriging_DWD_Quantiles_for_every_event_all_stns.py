...
path_to_netatmo_daily_edf_temp_filter = (
    main_dir / r'oridinary_kriging_compare_DWD_Netatmo' /
    r'all_netatmo__daily_ppt_edf__using_DWD_stations_to_find_Netatmo_values__temporal_filter_98perc_.csv')
# all_netatmo__daily_ppt_edf__using_DWD_stations_to_find_Netatmo_values__temporal_filter_98perc_
...
for event_date in dwd_in_extremes_df.index:
    ...
    for idx_lst_comb in range(len(shuffled_dwd_stns_10stn)):
        ...
        print('Interpolating for following DWD stations: \n',
              'Nbr of stations:', len(stn_comb),
              '\n Station ids:', pprint.pformat(stn_comb))
    # for stn_dwd_id in stn_comb:

        x_dwd_interpolate = dwd_in_coords_df.loc[stn_comb, 'X'].ravel()
        y_dwd_interpolate = dwd_in_coords_df.loc[stn_comb, 'Y'].ravel()

        # drop stn
        all_dwd_stns_except_interp_loc = [
            stn for stn in dwd_in_vals_df.columns if stn not in stn_comb]
        print('Using %d DWD stations for interpolation\n'
...
#                 interpolated_vals_netatmo_only = np.nan
        ...
        df_interpolated_dwd_netatmos_comb.loc[
            event_date, stn_comb] = interpolated_vals_dwd_netatmo

        df_interpolated_dwd_only.loc[
            event_date, stn_comb] = interpolated_vals_dwd_only

        df_interpolated_netatmo_only.loc[
            event_date, stn_comb] = interpolated_vals_netatmo_only
...
df_interpolated_dwd_netatmos_comb.to_csv(out_plots_path / (
    'interpolated_quantiles_all_dwd_stns_data_basedon_quantiles_%s_using_dwd_netamo_grp_.csv'
    % (time_res)),
    sep=';', float_format='%0.2f')

df_interpolated_dwd_only.to_csv(out_plots_path / (
    'interpolated_quantiles_all_dwd_stns_data_basedon_qunatiles_%s_using_dwd_only_grp.csv'
    % (time_res)),
    sep=';', float_format='%0.2f')

df_interpolated_netatmo_only.to_csv(out_plots_path / (
    'interpolated_quantiles_all_dwd_stns_data_basedon_qunatiles_%s_netatmo_only_grp.csv'
    % (time_res)),
    sep=';', float_format='%0.2f')
